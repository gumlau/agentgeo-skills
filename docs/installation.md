...
Get the ChatSights GEO Skills running inside your coding agent in three steps:
**connect the MCP → pick a mode → enable the skills.** Every skill fetches real AI
answers through the ChatSights `fetch_raw_answers` tool, then does the Generative
...
| **(Optional) A ChatSights API key** | Only needed for **live** data. Demo mode works with no key. | See [Step 2](#step-2--api-key--modes) |

```bash
# Verify Node and locate the repo
node --version                 # want v18.x or newer
cd /path/to/chatsights-geo-skills
pwd                            # copy this — it's your <ABSOLUTE-REPO-PATH>
ls mcp/index.mjs               # should print the path, no error
```

> Throughout this guide, replace `/absolute/path/to/chatsights-geo-skills` with the
...
## Step 1 — Connect the ChatSights MCP
...
|---------|------|---------|---------|
| API base URL | `--api-url <url>` | `CHATSIGHTS_API_URL` | `http://localhost:8080` |
| API key | `--key <cs_live_...>` | `CHATSIGHTS_API_KEY` | *(empty → demo mode)* |
...
```bash
claude mcp list                # chatsights should appear
```
...
```json
{
  "mcpServers": {
    "chatsights": {
      "command": "node",
      "args": ["/absolute/path/to/chatsights-geo-skills/mcp/index.mjs"],
      "env": {
        "CHATSIGHTS_API_URL": "https://api.trychatsights.com",
        "CHATSIGHTS_API_KEY": "cs_live_..."
      }
    }
  }
}
...
```toml
[mcp_servers.chatsights]
command = "node"
args = ["/absolute/path/to/chatsights-geo-skills/mcp/index.mjs"]

[mcp_servers.chatsights.env]
CHATSIGHTS_API_URL = "https://api.trychatsights.com"
CHATSIGHTS_API_KEY = "cs_live_..."
```
...
```bash
export CHATSIGHTS_API_URL="https://api.trychatsights.com"
export CHATSIGHTS_API_KEY="cs_live_..."     # omit for demo mode
node /absolute/path/to/chatsights-geo-skills/mcp/index.mjs
```
...
|------|--------------|--------------|------|
| **Demo** | No `--key` and no `CHATSIGHTS_API_KEY` | Labelled demo **fixtures** — real answer/citation *shape*, so every skill runs end to end | **0 credits** |
| **Live** | Yes | Real answers, citations, and sources from the six AI surfaces | Spends credits |

### Demo mode (default — start here)

Do nothing. With no key configured, ChatSights returns labelled demo fixtures at
...
1. Get a key at **[trychatsights.com](https://trychatsights.com)**. Keys are created
...
3. Pass the key via `--key` or `CHATSIGHTS_API_KEY`, and point `--api-url` at the
   hosted API.
...
export CHATSIGHTS_API_KEY="cs_live_..."
export CHATSIGHTS_API_URL="https://api.trychatsights.com"
claude mcp add chatsights -- node /absolute/path/to/chatsights-geo-skills/mcp/index.mjs
```
...
The agent invokes `geo-prompt-set` (the entry point), fetches through the ChatSights
...
Exact field names come from the ChatSights API; the point is that you get **raw
...
| **`Cannot find module …/index.mjs`** / server exits immediately | Relative or wrong path in the config | Use the **absolute** path from `pwd`. Confirm `ls /absolute/path/to/chatsights-geo-skills/mcp/index.mjs` succeeds. |
| **`401 Unauthorized`** in the fetch response | Auth is on (a key exists) but yours is missing or wrong | Pass `--key cs_live_...` or set `CHATSIGHTS_API_KEY`. Remember: creating the first key at `/app/keys` turns auth on for the workspace. |
...
| **`ChatSights API is unavailable at http://localhost:8080`** | No API listening at the `--api-url` target | Start the local API, or point `--api-url` / `CHATSIGHTS_API_URL` at the hosted URL (`https://api.trychatsights.com`). |
